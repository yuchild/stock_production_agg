...
def main():
    ...
    pred_label = label_mapping[pred].upper()
    ...
    # Left: text info with two sub-columns
    with col_left:
        ...
        with text_col2:
            ...
            st.markdown(f":arrow_forward: no_change: {probs[0]:.4f}")
            st.markdown(f":arrow_up: up: {probs[1]:.4f}")
            st.markdown(f":arrow_down: down: {probs[2]:.4f}")
    ...
    with col_right:
        ...
        df_hist = yf.Ticker(symbol).history(interval=interval,start=str(start_date),end=str(today),auto_adjust=False,prepost=True)
